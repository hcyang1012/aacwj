...
Scanner::Scanner(std::istream &input)
    : input_(input), put_back_(kNullChar), line_(1) {}

Token &Scanner::Curent() const { return *current_; }

size_t Scanner::GetLine() const { return line_; }
...
void Scanner::Scan() {
  ...
  switch (c) {
    ...
    case '+':
      current_ =
          std::make_unique<Token>(Token::Type::T_PLUS, std::to_string(c));
      break;
    case '-':
      current_ =
          std::make_unique<Token>(Token::Type::T_MINUS, std::to_string(c));
      break;
    case '*':
      current_ =
          std::make_unique<Token>(Token::Type::T_STAR, std::to_string(c));
      break;
    case '/':
      current_ =
          std::make_unique<Token>(Token::Type::T_SLASH, std::to_string(c));
      break;
    case ';':
      current_ =
          std::make_unique<Token>(Token::Type::T_SEMI, std::to_string(c));
      break;
    case '=':
      if ((c = next_token()) == '=') {
    ...
    default:
      if (is_digit(c)) {
        int n = scan_int(c);
        current_ =
            std::make_unique<Token>(Token::Type::T_INTLIT, std::to_string(n),
                                    std::make_unique<Value>(Value{n}));
...
void Scanner::set_put_back(const char c) { put_back_ = c; }

char Scanner::get_put_back() const { return put_back_; }
...
bool Scanner::is_digit(const char c) { return c >= '0' && c <= '9'; }
